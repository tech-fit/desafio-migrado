...
class ProfildPresenter (var id: Long){
    ...
    fun getPosts(onSuccess: (feed: Feed) -> Unit, onError: () -> Unit, id: Long, page: Int? = null, timestamp: Long? = null) {
        if (!requesting) {
            requesting = true

            //Chamada para a API
            ClientAPI.getService().getProfilePosts(id, page, timestamp).enqueue(object : Callback<Feed> {
    ...
    fun getMorePosts(onSuccess: (feed: Feed) -> Unit, onError: () -> Unit) {
        ...
        getPosts(onSuccess, onError, id, p, t)
    }
